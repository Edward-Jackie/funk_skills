...
将本模板复制为 `.codex/MAPCODE.md`。该文件是新任务的轻量路由入口，控制在 160 行以内。复杂领域必须拆分为同目录的 `.codex/MAPCODE-<领域>.md`，其中领域名使用小写英文、数字和连字符。禁止使用 `BUSINESS_MAP.md`、`docs/ai/`、`business/` 子目录或其他命名。
...
| --- | --- | --- | --- | --- | --- |
| <用户会说的任务、别名和实体> | [领域包](MAPCODE-<领域>.md) | <跨域包或无> | `<路径> -> <符号>` | <状态或规则> | <测试、日志、指标> |

## 业务域与核心链

| 业务域 | 核心业务链 | 业务成功结果 | 关键状态/实体 | 明细 |
| --- | --- | --- | --- | --- |
| <领域> | <触发到结果> | <价值结果> | `<实体.状态>` | [明细](MAPCODE-<领域>.md) |
