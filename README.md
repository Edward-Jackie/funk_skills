...
| 目录 | 用途 |
|---|---|
| `codemap/` | 生成 `CODEMAP.md` 补充 CLAUDE.md：任务型文件索引、并行调用链图、核心业务模块摘要。触发：/codemap、"梳理代码结构"、"怎么改 X" |
| `mapcode/` | 构建跨 AI 共用的业务规则与实现地图：核心业务链、不变量、状态机、幂等、补偿、代码证据与验证边界。适合接手复杂项目和高风险改动（金额/权限/库存/额度） |
